...
class CachedPathTest extends AbstractRuleTest
{
    public function setup()
    {
        parent::setup();
        $this->rule = new CachedPath();
    }
    ...
    {
        $this->rule = new CachedPath('/path/to/sub/index.php');
        $proto = $this->newRoute('/foo/bar/baz');
    ...
    public function testPatternCaching()
    {
        $this->rule = new CachedPath();
        $route = $this->newRoute('/foo/{id}')
            ->tokens(['id' => '\d+']);

        // First match - should build and cache the pattern
        $request = $this->newRequest('/foo/123');
        $this->assertIsMatch($request, $route);
        $this->assertEquals(1, $this->rule->getCacheSize());

        // Second match with same route - should use cached pattern
        $route2 = clone $route;
        $request2 = $this->newRequest('/foo/456');
        $this->assertIsMatch($request2, $route2);
        $this->assertEquals(1, $this->rule->getCacheSize());

        // Different route - should create new cache entry
        $route3 = $this->newRoute('/bar/{id}')
            ->tokens(['id' => '\d+']);
        $request3 = $this->newRequest('/bar/789');
        $this->assertIsMatch($request3, $route3);
        $this->assertEquals(2, $this->rule->getCacheSize());
    }
    ...
    public function testClearCache()
    {
        ...
        $this->rule->clearCache();
        $this->assertEquals(0, $this->rule->getCacheSize());
    }
}
